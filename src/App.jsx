...
function CanadaLeafIcon({ size = 16 }) {
  return <svg width={size} height={size} viewBox="0 0 650 650" aria-label="Available in Canada" role="img"><path fill="red" d="M166.21,550.25c-2.19,6.04,2.97,12.19,9.3,11.08l124.33-21.85c1.09-.19,2.2-.26,3.3-.21,8.45.39,14.99,7.56,14.6,16.01l-1.09,20.82c-.25,4.78,3.56,8.79,8.34,8.79h0c4.79,0,8.59-4.01,8.34-8.79l-1.09-20.82c-.05-1.11.02-2.21.21-3.3,1.43-8.34,9.36-13.94,17.7-12.5l124.33,21.85c6.33,1.11,11.49-5.04,9.3-11.08l-13.79-38.04c-1.56-4.21-.26-8.94,3.23-11.77l141.39-114.49c4.8-3.88,3.87-11.45-1.72-14.06l-22.09-10.31c-4.78-2.26-7.13-7.72-5.48-12.74l25.53-78.5c1.99-6.12-3.39-12.09-9.68-10.76l-73.27,15.55c-4.88,1.02-9.81-1.55-11.77-6.13l-11.9-28c-2.37-5.57-9.68-6.85-13.8-2.43l-59.46,63.81c-2.46,2.6-6.08,3.76-9.59,3.08-5.69-1.09-9.41-6.59-8.31-12.28l29.41-151.68c1.36-7.03-6.18-12.41-12.38-8.82l-36.87,21.31c-.2.12-.4.23-.61.33-5.18,2.59-11.48.49-14.07-4.69l-46.1-90.54c-3.1-6.08-11.79-6.08-14.89,0l-46.1,90.54c-.1.21-.21.41-.33.61-2.93,5-9.35,6.67-14.35,3.74l-36.87-21.31c-6.2-3.59-13.75,1.79-12.38,8.82l29.41,151.68c.68,3.51-.49,7.13-3.08,9.59-4.2,3.98-10.84,3.81-14.82-.4l-59.46-63.81c-4.12-4.42-11.44-3.14-13.8,2.43l-11.9,28c-1.97,4.58-6.89,7.15-11.77,6.13l-73.27-15.55c-6.29-1.33-11.67,4.64-9.68,10.76l25.53,78.5c1.64,5.02-.71,10.48-5.48,12.74l-22.09,10.31c-5.59,2.61-6.52,10.18-1.72,14.06l141.39,114.49c3.49,2.83,4.78,7.56,3.23,11.77l-13.79,38.04Z" /></svg>;
}

function CanadaBadge({ country }) {
  return country === "CA" ? <CanadaLeafIcon /> : null;
}

function Icon({ type, active = true, size = 24 }) {
  const color = active ? "#000" : "#737373";
  ...
  if (type === "scan") return <svg {...common}><path d="M3 8V4h4" /><path d="M17 4h4v4" /><path d="M21 16v4h-4" /><path d="M7 20H3v-4" /><line x1="6" y1="7" x2="6" y2="17" strokeWidth="2.6" /><line x1="9" y1="7" x2="9" y2="17" strokeWidth="1" /><line x1="11.5" y1="7" x2="11.5" y2="17" strokeWidth="2.8" /><line x1="14.5" y1="7" x2="14.5" y2="17" strokeWidth="1.2" /><line x1="17.5" y1="7" x2="17.5" y2="17" strokeWidth="2.6" /></svg>;
  if (type === "search") return <svg {...common}><circle cx="11" cy="11" r="6" /><line x1="16" y1="16" x2="21" y2="21" /></svg>;
  if (type === "history") return <svg {...common}><path d="M4 12a8 8 0 1 0 2.3-5.7" /><polyline points="4 4 4 9 9 9" /><circle cx="12" cy="12" r="1" fill={color} stroke="none" /><path d="M12 12V7" /><path d="M12 12l4 2" /></svg>;
  if (type === "social") return <svg {...common}><path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12z" /><circle cx="12" cy="12" r="3" /></svg>;
  if (type === "profile") return <svg {...common}><circle cx="12" cy="8" r="4" /><path d="M4 20c2-4 14-4 16 0" /></svg>;
  if (type === "bell") return <svg {...common}><path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 7h18s-3 0-3-7" /><path d="M13.73 21a2 2 0 0 1-3.46 0" /></svg>;
  return null;
}

function Button({ children, onClick, variant = "solid", className = "" }) {
  const styles = { solid: "bg-neutral-950 text-white hover:bg-neutral-800", outline: "border border-neutral-200 bg-white text-neutral-950 hover:bg-neutral-50", ghost: "bg-transparent text-neutral-600 hover:bg-neutral-100", light: "bg-white text-neutral-950 hover:bg-neutral-50" };
  return <button type="button" onClick={onClick} className={`inline-flex items-center justify-center rounded-full px-4 py-2 text-sm font-medium transition ${styles[variant] || styles.solid} ${className}`}>{children}</button>;
}

function ToggleSwitch({ checked, onClick, label }) {
  return <button type="button" onClick={onClick} className={`relative h-8 w-14 shrink-0 rounded-full transition ${checked ? "bg-neutral-950" : "bg-neutral-300"}`} aria-label={label}><span className={`absolute left-1 top-1 h-6 w-6 rounded-full bg-white shadow-sm transition-transform ${checked ? "translate-x-6" : "translate-x-0"}`} /></button>;
}

const Card = ({ children, className = "" }) => <div className={`rounded-3xl border-0 bg-white shadow-sm ${className}`}>{children}</div>;
const Header = ({ title, right }) => <div className="flex items-center justify-between px-5 pb-3 pt-6"><h1 className="text-2xl font-semibold tracking-tight text-neutral-950">{title}</h1>{right || <span />}</div>;
const Phone = ({ children }) => <div className="mx-auto min-h-[760px] w-full max-w-[430px] overflow-hidden rounded-[2rem] border border-neutral-200 bg-[#f7f3eb] shadow-2xl">{children}</div>;
...
function ScoreRing({ score }) {
  ...
  return <div className="relative flex h-[154px] w-[154px] items-center justify-center"><svg width="154" height="154" viewBox="0 0 140 140" className="-rotate-90"><circle cx="70" cy="70" r={radius} stroke="#ece8df" strokeWidth="13" fill="none" /><motion.circle cx="70" cy="70" r={radius} stroke={theme.ring} strokeWidth="13" fill="none" strokeLinecap="round" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} animate={{ strokeDashoffset: offset }} transition={{ duration: 0.8 }} /></svg><div className="absolute text-center"><div className="text-4xl font-semibold tracking-tight text-neutral-950">{value}</div><div className="text-xs text-neutral-500">/ 100</div></div></div>;
}

function ProductRow({ product, onClick }) {
  return <button type="button" onClick={onClick} className="w-full text-left"><Card className="bg-white/80"><div className="flex items-center gap-3 p-3"><ProductImage src={product.imageUrl} alt={product.name} className="h-14 w-14 rounded-xl object-cover" /><div className="min-w-0 flex-1"><div className="flex items-center gap-1 truncate font-medium text-neutral-950"><span className="truncate">{product.name}</span><CanadaBadge country={product.country} /></div><div className="text-sm text-neutral-500">{product.brand}</div></div><div className="flex h-12 w-12 items-center justify-center rounded-full text-sm font-semibold" style={{ color: product.theme.ring, background: product.theme.bg }}>{product.score}</div></div></Card></button>;
}

function BottomNav({ tab, setTab }) {
  return <div className="grid grid-cols-5 border-t border-neutral-200 bg-[#fbf8f2] px-1 py-2">{getBottomNavItems().map(([key, type, label]) => <button type="button" key={key} onClick={() => setTab(key)} className="flex flex-col items-center gap-1 rounded-xl px-1 py-2 text-xs"><Icon type={type} active={tab === key} /><span className={tab === key ? "font-medium text-neutral-950" : "text-neutral-500"}>{label}</span></button>)}</div>;
}

function ScanScreen({ products, openResult }) {
  const [flashOn, setFlashOn] = useState(false);
  return <div className="relative flex min-h-[690px] flex-col overflow-hidden bg-neutral-950 text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_#3f3f46_0%,_#18181b_55%,_#09090b_100%)]" /><div className="relative z-10 flex items-center justify-between px-5 pb-3 pt-6"><h1 className="text-2xl font-semibold tracking-tight">Scan product barcode</h1><button type="button" onClick={() => setFlashOn(!flashOn)} className={`flex h-11 w-11 items-center justify-center rounded-full border border-white/20 ${flashOn ? "bg-white text-neutral-950" : "bg-white/10 text-white"}`} aria-label="Toggle flashlight"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18h6" /><path d="M10 22h4" /><path d="M10 18c0-2-3-3.5-3-7a5 5 0 0 1 10 0c0 3.5-3 5-3 7" /><path d="M12 2v2" /><path d="M4.9 4.9l1.4 1.4" /><path d="M19.1 4.9l-1.4 1.4" /></svg></button></div><div className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 text-center"><button type="button" onClick={() => openResult(products[4] || null)} className="relative flex h-64 w-64 items-center justify-center rounded-[2rem] border-2 border-white/80 bg-white/5 shadow-2xl"><span className="absolute left-8 top-8 h-8 w-8 border-l-4 border-t-4 border-white" /><span className="absolute right-8 top-8 h-8 w-8 border-r-4 border-t-4 border-white" /><span className="absolute bottom-8 left-8 h-8 w-8 border-b-4 border-l-4 border-white" /><span className="absolute bottom-8 right-8 h-8 w-8 border-b-4 border-r-4 border-white" /><Icon type="scan" size={112} active={false} /></button><p className="mt-6 max-w-[280px] text-sm leading-6 text-white/70">Point your camera at a barcode to check for hidden plastic.</p><Button onClick={() => openResult(null)} variant="light" className="mt-6">Simulate unknown barcode</Button></div></div>;
}
...
function PlansScreen({ close }) {
  ...
  return <div className="min-h-[690px] overflow-y-auto px-5 pb-5"><Header title="Plans" right={<Button onClick={close} variant="ghost">Back</Button>} /><div className="mb-5 grid grid-cols-2 rounded-full bg-white p-1 shadow-sm"><button type="button" onClick={() => setBilling("monthly")} className={`rounded-full py-2 text-sm font-medium ${billing === "monthly" ? "bg-neutral-950 text-white" : "text-neutral-500"}`}>Monthly</button><button type="button" onClick={() => setBilling("yearly")} className={`rounded-full py-2 text-sm font-medium ${billing === "yearly" ? "bg-neutral-950 text-white" : "text-neutral-500"}`}>Yearly</button></div><div className="space-y-4"><Card><div className="p-5"><div className="flex items-start justify-between"><div><h2 className="text-2xl font-semibold tracking-tight text-neutral-950">Free</h2><p className="mt-1 text-sm text-neutral-500">For casual product checks.</p></div><div className="rounded-full bg-[#f7f3eb] px-3 py-1 text-sm font-medium text-neutral-700">$0</div></div><div className="mt-5 space-y-3">{freeFeatures.map((feature) => <div key={feature} className="flex gap-2 text-sm text-neutral-700"><span>✓</span><span>{feature}</span></div>)}</div><Button variant="outline" className="mt-5 w-full bg-white">Current plan</Button></div></Card><div className="rounded-3xl bg-neutral-950 p-5 text-white shadow-sm"><div className="flex items-start justify-between"><div><div className="mb-2 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-medium text-white">Best value</div><h2 className="text-2xl font-semibold tracking-tight">Pro</h2><p className="mt-1 text-sm text-neutral-300">For people actively reducing plastic exposure.</p></div><div className="text-right"><div className="text-xl font-semibold">{proPrice}</div>{billing === "yearly" && <div className="text-xs text-neutral-400">Save 33%</div>}</div></div><div className="mt-5 space-y-3">{proFeatures.map((feature) => <div key={feature} className="flex gap-2 text-sm text-neutral-200"><span>✓</span><span>{feature}</span></div>)}</div><Button variant="light" className="mt-5 w-full">Start Pro</Button></div></div></div>;
}
...
function BadgeCard({ badge, highlight, compact = false }) {
  ...
  return <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className={`relative rounded-[24px] bg-[#e9e3da] shadow-sm ${compact ? "min-h-[144px] p-3" : "min-h-[210px] p-4"}`}><motion.div animate={highlight ? { scale: [1, 1.15, 1] } : { scale: 1 }} transition={{ duration: 0.6 }} className={`absolute flex items-center justify-center rounded-full text-white ${compact ? "right-3 top-3 h-7 w-7 text-xs" : "right-5 top-5 h-10 w-10 text-lg"}`} style={{ backgroundColor: medalColor }}>★{highlight && <motion.div initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 0] }} transition={{ duration: 1 }} className="absolute inset-0 rounded-full bg-white/40 blur-sm" />}</motion.div><div className="flex flex-col items-center text-center"><div className={`flex items-center justify-center rounded-full bg-white ${compact ? "mt-4 h-16 w-16 text-3xl" : "mt-6 h-24 w-24 text-5xl"}`}>{badge.icon}</div><h4 className={`font-semibold text-neutral-950 ${compact ? "mt-3 text-sm leading-tight" : "mt-5 text-2xl leading-tight"}`}>{badge.name}</h4><p className={`mx-auto text-neutral-600 ${compact ? "mt-1 line-clamp-3 min-h-[48px] max-w-[150px] text-[11px] leading-4" : "mt-3 line-clamp-3 min-h-[72px] max-w-[320px] text-lg leading-snug"}`}>{badge.description}</p></div><div className={compact ? "mt-2" : "mt-4"}><div className={`mb-1 flex justify-between text-neutral-700 ${compact ? "text-xs" : "text-lg"}`}><span>{status.currentTier}</span><span>{progressText}</span></div><div className={`overflow-hidden rounded-full bg-white ${compact ? "h-2" : "h-4"}`}><motion.div className="h-full rounded-full bg-black" animate={{ width: `${status.percent}%` }} transition={{ duration: 0.5 }} /></div></div></motion.div>;
}
